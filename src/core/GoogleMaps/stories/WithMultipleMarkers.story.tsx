import React from 'react';
import { GoogleMaps } from '../GoogleMaps';
import { GoogleMapsMarker } from '@vanguard/GoogleMaps';
import { Story, testMapOptions } from './_GoogleMaps.default';

const markers = [
  { id: 'marker-1', lat: 40.7128, lng: -74.006 },
  { id: 'marker-2', lat: 40.7589, lng: -73.9851 },
  { id: 'marker-3', lat: 40.6782, lng: -73.9442 },
  { id: 'marker-4', lat: 40.7489, lng: -73.9680 },
];

export const WithMultipleMarkers: Story = {
  args: {
    center: testMapOptions.center,
    zoom: testMapOptions.zoom,
  },
  render: (args) => (
    <GoogleMaps {...args}>
      {markers.map((marker) => (
        <GoogleMapsMarker
          key={marker.id}
          id={marker.id}
          isJsApiLoaded={true}
          pos={{ lat: marker.lat, lng: marker.lng }}
          icon="data:image/png;base64,"
          size={{ x: 28, y: 35 }}
          showInfoBox={false}
          onClick={(id) => console.log('Marker clicked:', id)}
        />
      ))}
    </GoogleMaps>
  ),
  parameters: {
    docs: {
      description: {
        story: 'Google Maps component displaying multiple markers at different locations.',
      },
    },
  },
};
